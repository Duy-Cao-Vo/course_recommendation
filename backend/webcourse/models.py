...
class User(StructuredNode):
    user = RelationshipTo('MyUser', 'RELATED_TO')
    uid = IntegerProperty()
    name = StringProperty()
...
class Course(StructuredNode):
    ...
    def export(self):
        ...
        return {
            ...
            "require_knowledge":list( map(lambda n: n.export(), self.require_knowledge)),
            "require_platform": list(map(lambda n: n.export(), self.require_platform)),
            "require_tool": list(map(lambda n: n.export(), self.require_tool)),
            "require_programinglanguage": list(map(lambda n: n.export(), self.require_programinglanguage)),
            "require_framework": list(map(lambda n: n.export(), self.require_framework)),
...
def Career_Course(career):
    similar=career.cypher("CALL gds.nodeSimilarity.stream('graph') YIELD node1, node2, similarity  where gds.util.asNode(node1).creTitle='"+career.creTitle+"' and gds.util.asNode(node2).creTitle is  null  RETURN gds.util.asNode(node1).creTitle AS Career, gds.util.asNode(node2).crsLink as Course, similarity")
    return similar[0]
